...
class Model:
    def __init__(self, input_steps, embedding_size, hidden_size, vocab_size, slot_size,
                 intent_size, epoch_num, batch_size=64):
        ...
        self.output_embeddings = tf.get_variable(initializer=tf.random_uniform([self.vocab_size, self.embedding_size],
                                                                               -0.1, 0.1), dtype=tf.float32, name="output_embeddings")
    ...
    def build(self):
        with tf.variable_scope('embedding_layer'):
            self.encoder_inputs = tf.placeholder(tf.float32, [self.input_steps, self.batch_size, self.embedding_size],
                                                 name='encoder_inputs')
        ...
        with tf.variable_scope('decoder_layer'):
            ...
            with tf.variable_scope('helper_function'):
                ...
                sos_step_embedded = tf.nn.embedding_lookup(self.output_embeddings, sos_time_slice)
                ...
                def next_inputs_fn(time, outputs, state, sample_ids):
                    ...
                    pred_embedding = tf.nn.embedding_lookup(self.output_embeddings, sample_ids)
                    # 输入是h_i+o_{i-1}+c_i
    ...
    def step(self, sess, trarin_batch, is_Test=False):
        ...
        :param sess:
        :param trarin_batch:
        :param is_Test: 如果 is_Test 为真，则为测试模式。这时候不会去进行梯度下降(train_op)了。
        :return: 返回值 看 feed_dict 中的选项。
        """
        if is_Test is False:
